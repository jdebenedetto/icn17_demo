...
#include "icnet_http_client_connection_said.h"
...
HTTPClientConnectionSaid::HTTPClientConnectionSaid()
    : consumer_(Name("ccnx:"), transport::TransportProtocolAlgorithms::SAID) {
  ...
  consumer_.setSocketOption(ConsumerCallbacksOptions::CONTENT_OBJECT_TO_VERIFY,
                            (ConsumerContentObjectVerificationCallback) std::bind(&HTTPClientConnectionSaid::verifyData,
                                                                                  this,
                                                                                  std::placeholders::_1,
                                                                                  std::placeholders::_2));

  consumer_.setSocketOption(ConsumerCallbacksOptions::CONTENT_RETRIEVED,
                            (ConsumerContentCallback) std::bind(&HTTPClientConnectionSaid::processPayload,
                                                                this,
                                                                std::placeholders::_1,
                                                                std::placeholders::_2));
}

HTTPClientConnectionSaid &HTTPClientConnectionSaid::get(std::string &url, HTTPHeaders headers, HTTPPayload payload) {
  ...
  consumer_.setSocketOption(ConsumerCallbacksOptions::INTEREST_OUTPUT,
                            (ConsumerInterestCallback) std::bind(&HTTPClientConnectionSaid::processLeavingInterest,
                                                                 this,
                                                                 std::placeholders::_1,
                                                                 std::placeholders::_2,
                                                                 request_string));
...
HTTPResponse &&HTTPClientConnectionSaid::response() {
  return std::move(response_);
}

void HTTPClientConnectionSaid::processPayload(transport::ConsumerSocket &c,
                                          std::vector<uint8_t> &&payload) {
  response_ = std::move(payload);
}

bool HTTPClientConnectionSaid::verifyData(transport::ConsumerSocket &c, const ContentObject &contentObject) {
...
void HTTPClientConnectionSaid::processLeavingInterest(transport::ConsumerSocket &c,
...
HTTPClientConnectionSaid& HTTPClientConnectionSaid::stop() {
