...
LONGBOW_TEST_FIXTURE(Global)
{
    ...
    LONGBOW_RUN_TEST_CASE(Global, ccnxInterest_SetGetFlags);
...
LONGBOW_TEST_CASE(Global, ccnxInterest_SetGetFlags)
{
    CCNxName *name = ccnxName_CreateFromCString("lci:/name");
    CCNxInterest *interest = ccnxInterest_CreateSimple(name);
    ...
    if (impl->getFlags) {
        assertTrue(ccnxInterest_GetFlags(interest) == CCNxInterestDefault_Flags, "Expected the default flags value");
    }

    if (impl->setFlags && impl->getFlags) {
        ccnxInterest_SetFlags(interest, 10);
        assertTrue(ccnxInterest_GetFlags(interest) == 10, "Expected a flags of 10");
        ccnxInterest_SetFlags(interest, 20);
        assertTrue(ccnxInterest_GetFlags(interest) == 20, "Expected a flags of 20");
    }
...
_commonSetup(void)
{
    ...
    data->name = name;
    data->interest = ccnxInterest_CreateWithImplFlag(&data->impl, name, 100, NULL, NULL, 10, 15);
