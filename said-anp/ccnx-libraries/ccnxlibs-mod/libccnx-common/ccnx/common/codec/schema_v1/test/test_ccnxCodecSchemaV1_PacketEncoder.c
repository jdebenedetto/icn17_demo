...
LONGBOW_TEST_CASE(Interest, v1_interest_nameA_crc32c)
{
    ...
    CCNxTlvDictionary *message =
        ccnxInterest_CreateWithImplFlag(&CCNxInterestFacadeV1_Implementation,
                                    name, CCNxInterestDefault_LifetimeMilliseconds, NULL, NULL, 32, 17);
...
LONGBOW_TEST_FIXTURE(Local)
{
    ...
    LONGBOW_RUN_TEST_CASE(Local, _getFlags_Present);
    LONGBOW_RUN_TEST_CASE(Local, _getFlags_Missing);
...
LONGBOW_TEST_CASE(Local, _getFlags_Present)
{
    uint8_t flags = 15;
    CCNxTlvDictionary *dict = ccnxCodecSchemaV1TlvDictionary_CreateInterest();
    ccnxTlvDictionary_PutInteger(dict, CCNxCodecSchemaV1TlvDictionary_MessageFastArray_FLAGS, flags);

    uint8_t test = _getFlags(dict);
    assertTrue(test == flags, "Got wrong flags, expected %u got %u", flags, test);
    ccnxTlvDictionary_Release(&dict);
}

LONGBOW_TEST_CASE(Local, _getFlags_Missing)
{
    CCNxTlvDictionary *dict = ccnxCodecSchemaV1TlvDictionary_CreateInterest();

    uint8_t test = _getFlags(dict);
    assertTrue(test == CCNxInterestDefault_Flags, "Got wrong flags, expected %u got %u", CCNxInterestDefault_Flags, test);
    ccnxTlvDictionary_Release(&dict);
}
...
LONGBOW_TEST_CASE(Local, _encodeFixedHeader_Interest)
{
    uint8_t encoded[] = {
        ...
        0x1f, 0x00, 0x11, 14,       // hoplimit = 31,  reserved = 0x0000, flags = 17, header length = 14
    };
    PARCBuffer *truth = parcBuffer_Wrap(encoded, sizeof(encoded), 0, sizeof(encoded));

    CCNxTlvDictionary *dict = ccnxCodecSchemaV1TlvDictionary_CreateInterest();
    ccnxTlvDictionary_PutInteger(dict, CCNxCodecSchemaV1TlvDictionary_MessageFastArray_HOPLIMIT, 31);
    ccnxTlvDictionary_PutInteger(dict, CCNxCodecSchemaV1TlvDictionary_MessageFastArray_FLAGS, 17);
