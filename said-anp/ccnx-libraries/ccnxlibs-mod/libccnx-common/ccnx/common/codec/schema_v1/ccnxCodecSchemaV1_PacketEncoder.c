...
static uint8_t
_getFlags(CCNxTlvDictionary *packetDictionary)
{
    uint8_t flags = (uint8_t) CCNxInterestDefault_Flags;

    if (ccnxTlvDictionary_IsValueInteger(packetDictionary, CCNxCodecSchemaV1TlvDictionary_MessageFastArray_FLAGS)) {
        flags = (uint8_t) ccnxTlvDictionary_GetInteger(packetDictionary, CCNxCodecSchemaV1TlvDictionary_MessageFastArray_FLAGS);
    }
    return flags;
}
                   ...
                   ssize_t packetLength)
{
    ...
    if ((packetType == CCNxCodecSchemaV1Types_PacketType_Interest) ||
        ...
        interestHeader->flags = _getFlags(packetDictionary);
