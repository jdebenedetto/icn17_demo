...
CCNxInterest *ccnxInterest_CreateWithImplFlag(const CCNxInterestInterface *implementation,
                                          const CCNxName *name,
                                          const uint32_t interestLifetime,
                                          const PARCBuffer *keyId,
                                          const PARCBuffer *contentObjectHash,
...
/**
 * Set the flag at the selected index in the Flags field for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit value in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to a `CCNxInterest` instance to update.
 * @param [in] bit offset [0 ... 7] of the flag to be set (== 1)
 * @return TRUE if the flag was unset.
 ...
 *     CCNxInterest *interest = ccnxInterest_CreateSimple(name);
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = false, "Expected false for the flag value");
 *     
 *     ccnxInterest_SetFlag(interest, 2);
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = 1, "Expected 1 for the flag value");
 *
 *     ccnxName_Release(&name);
 *     ccnxInterest_Release(&instance);
 * }
 * @endcode
 * @see `ccnxInterest_GetFlag`
 */
// bool ccnxInterest_SetFlag(CCNxInterest *interest, unsigned index);

/**
 * Clear the flag at the selected index in the Flags field for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit value in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to a `CCNxInterest` instance to update.
 * @param [in] bit offset [0 ... 7] of the flag to be unset (== 0) 
 * @return TRUE if the flag is succesfully cleared,
 *         FALSE if the flag was already cleared.
 *
 * Example:
 * @code
 * {
 ...
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = 1, "Expected the flag to be set (= 1)");
 *
 *     ccnxInterest_ClearFlag(interest, 2);
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = 0, "Expected the flag to be cleared (= 0)");
 *
 *     ccnxName_Release(&name);
 *     ccnxInterest_Release(&instance);
 * }
 ...
 * @see `ccnxInterest_GetFlag`
 */
// bool ccnxInterest_ClearFlag(CCNxInterest *interest, unsigned index);

/**
 * Get the flag at the selected index in the Flags field for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit value in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to a `CCNxInterest` instance to update.
 * @param [in] bit offset [0 ... 7] of the flag to be set (== 1)
 ...
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = 0, "Expected 0 for the flag value");
 *     
 *     ccnxInterest_SetFlag(interest, 2);
 *
 *     int flag = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag = 1, "Expected 1 for the flags value");
 *
 *     ccnxName_Release(&name);
 *     ccnxInterest_Release(&instance);
 * }
 * @endcode
 * @see `ccnxInterest_SetFlag`
 */
// int ccnxInterest_GetFlag(const CCNxInterest *interest, unsigned index);

/**
 * Clear all the flags in the Flags field for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit value in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to a `CCNxInterest` instance to update.
 * 
 ...
 *     
 *     ccnxInterest_SetFlag(interest, 2);
 *     ccnxInterest_SetFlag(interest, 5);
 *     
 *
 *     int flag2 = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag2 = true, "Expected true for the flags value");
 *     int flag5 = ccnxInterest_GetFlag(interest, 5);
 *     assertTrue(flag2 = true, "Expected true for the flags value");
 *     
 *     ccnxInterest_resetFlags(const CCNxInterest *interest);
 *     int flag2 = ccnxInterest_GetFlag(interest, 2);
 *     assertTrue(flag2 = false, "Expected false for the flags value");
 *     int flag5 = ccnxInterest_GetFlag(interest, 5);
 *     assertTrue(flag2 = false, "Expected false for the flags value");
 *
 *     ccnxName_Release(&name);
 ...
 * @see `ccnxInterest_SetFlag`
 */
// bool ccnxInterest_ResetFlags(const CCNxInterest *interest);

/**
 * Set the Flags header for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit value in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to a `CCNxInterest` instance to update.
 * @param [in] flags The flags value to set.
 *
 * Example:
 * @code
 * {
 ...
 *     CCNxInterest *interest = ccnxInterest_CreateSimple(name);
 *
 *     ccnxInterest_SetFlags(interest, 5);
 *
 *     uint32_t flags = ccnxInterest_GetFlags(interest);
 *     assertTrue(flags = 5, "Expected 5 for the hopLimit");
 *
 *     ccnxName_Release(&name);
 *     ccnxInterest_Release(&instance);
 * }
 * @endcode
 * @see `ccnxInterest_GetFlags`
 */
bool ccnxInterest_SetFlags(CCNxInterest *interest, uint32_t flags);

/**
 * Get the Flags header for the specified `CCNxInterest` instance.
 * The Interest Flags element is an 8bit array in the Interest fixed header that can be used for application specific flags.
 *
 * @param [in] interest A pointer to the `CCNxInterest` from which to retrieve the Flags field
 *
 * Example:
 * @code
 * {
 ...
 * }
 * @endcode
 * @see `ccnxInterest_SetFlags`
 */
uint32_t ccnxInterest_GetFlags(const CCNxInterest *interest);
