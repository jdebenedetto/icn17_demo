...
LONGBOW_TEST_FIXTURE(Global)
{
    ...
    LONGBOW_RUN_TEST_CASE(Global, ccnxInterestFacadeV1_GetFlags);
...
LONGBOW_TEST_CASE(Global, ccnxInterestFacadeV1_GetFlags)
{
    TestData *data = longBowTestCase_GetClipBoardData(testCase);
    unsigned test = (unsigned) _ccnxInterestFacadeV1_GetFlags(data->interest);
    assertTrue(test == data->flags, "Wrong flags: got %u expected %u", test, data->flags); //need first to add flags field to the Interest create operation
    //assertTrue(test == 0, "Wrong flags: got %u expected %u", test, 0);
}
