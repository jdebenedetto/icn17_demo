...
//---------------------SAID-----------------------------------------------------------------------
Name Name::getRandomNextSegmentName(int max_rand) const {

  size_t name_size = ccnxName_GetSegmentCount(name_);
  size_t begin = 0;

  ssize_t start_component = 0;
  ssize_t number_of_components = name_size - 1;
  ...
  ret.appendSegment(this->get(-1).toSegment() + (rand() % max_rand));

  return ret;
}

Name Name::getANPName() const {
  ...
  // size_t number_of_components = this->getSegmentCount();
  size_t number_of_components = ccnxName_GetSegmentCount(name_);
...
// Name &Name::appendANPSegment(const uint64_t chunk_number) {
//   CCNxNameSegmentStructure *ns = ccnxNameSegmentNumber_Create(CCNxNameLabelType_ANP, chunk_number);
//   name_ = ccnxName_Append(name_, ns);
//   ccnxNameSegment_Release(&ns);
