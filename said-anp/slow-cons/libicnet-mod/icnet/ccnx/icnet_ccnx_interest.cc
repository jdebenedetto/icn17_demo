...
void Interest::setFlags(uint32_t flags) {
  ccnxInterest_SetFlags(interest_, flags);
}

uint32_t Interest::getFlags() {
  return ccnxInterest_GetFlags(interest_);
}

bool Interest::hasFlag(uint32_t flag) {
  if(ccnxInterest_GetFlags(interest_) == flag) {
    return true;
  } else {
    return false;
  }
