...
const std::shared_ptr<ContentObject> &ContentStore::find(const Interest &interest) {
  std::unique_lock<std::mutex> lock(cs_mutex_);
  ContentStoreHashTable::iterator it = content_store_hash_table_.find(interest.getName()); //TO RESTORE AFTER DEMO
  //----------------SAID-TEST------------------------------------------------------------------------------------------------
  if(interest.hasFlag(1)) {
    // it = content_store_hash_table_.find(interest.getName().getRandomNextSegmentName(3));
  }
  //----------------SAID-TEST------------------------------------------------------------------------------------------------
