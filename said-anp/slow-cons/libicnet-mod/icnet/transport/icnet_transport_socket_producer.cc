...
void ProducerSocket::onInterest(const Name &name, const Interest &interest) {
  ...
  std::cout << "Receive request for segment: " << interest.getName().get(-1).toSegment() << std::endl; //DEBUG 
  ...
  if (content_object) {
    ...
    // std::cout << "Sending segment: " << content_object->getName().get(-1).toSegment() << std::endl; //DEBUG 
    portal_->sendContentObject(*content_object);
  } else {
