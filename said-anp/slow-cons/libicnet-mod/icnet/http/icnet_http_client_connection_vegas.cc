...
#include "icnet_http_client_connection_vegas.h"
...
HTTPClientConnectionVegas::HTTPClientConnectionVegas()
    : consumer_(Name("ccnx:"), transport::TransportProtocolAlgorithms::VEGAS) {
  ...
  consumer_.setSocketOption(GeneralTransportOptions::MAX_INTEREST_RETX, 100000000);

  consumer_.setSocketOption(ConsumerCallbacksOptions::CONTENT_OBJECT_TO_VERIFY,
                            (ConsumerContentObjectVerificationCallback) std::bind(&HTTPClientConnectionVegas::verifyData,
                                                                                  this,
                                                                                  std::placeholders::_1,
                                                                                  std::placeholders::_2));

  consumer_.setSocketOption(ConsumerCallbacksOptions::CONTENT_RETRIEVED,
                            (ConsumerContentCallback) std::bind(&HTTPClientConnectionVegas::processPayload,
                                                                this,
                                                                std::placeholders::_1,
                                                                std::placeholders::_2));
}

HTTPClientConnectionVegas &HTTPClientConnectionVegas::get(std::string &url, HTTPHeaders headers, HTTPPayload payload) {
  ...
  consumer_.setSocketOption(ConsumerCallbacksOptions::INTEREST_OUTPUT,
                            (ConsumerInterestCallback) std::bind(&HTTPClientConnectionVegas::processLeavingInterest,
                                                                 this,
                                                                 std::placeholders::_1,
                                                                 std::placeholders::_2,
                                                                 request_string));
...
HTTPResponse &&HTTPClientConnectionVegas::response() {
  return std::move(response_);
}

void HTTPClientConnectionVegas::processPayload(transport::ConsumerSocket &c,
                                          std::vector<uint8_t> &&payload) {
  response_ = std::move(payload);
}

bool HTTPClientConnectionVegas::verifyData(transport::ConsumerSocket &c, const ContentObject &contentObject) {
...
void HTTPClientConnectionVegas::processLeavingInterest(transport::ConsumerSocket &c,
...
HTTPClientConnectionVegas& HTTPClientConnectionVegas::stop() {
