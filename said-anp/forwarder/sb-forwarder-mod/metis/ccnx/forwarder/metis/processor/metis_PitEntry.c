...
metisPitEntry_Create(MetisMessage *message, MetisTicks expiryTime, MetisTicks creationTime)
{
    ...
    if(flags == 1) {
        metisNumberSet_IncCount(pitEntry->ingressIdSet, metisMessage_GetIngressConnectionId(message));
    }
    else {
        metisNumberSet_Add(pitEntry->ingressIdSet, metisMessage_GetIngressConnectionId(message));
    }
...
//----------------SAID----------------------------------------------------
bool
metisPitEntry_AddOrUpdateIngressId(MetisPitEntry *pitEntry, unsigned ingressId)
{
    assertNotNull(pitEntry, "Parameter pitEntry must be non-null");
    return metisNumberSet_IncCount(pitEntry->ingressIdSet, ingressId);
}

bool
metisPitEntry_AfterSatisfy(MetisPitEntry *pitEntry)
{
    assertNotNull(pitEntry, "Parameter pitEntry must be non-null");
    // Dec PRmax
    // If PRmax == 0 return true (remove PITentry),
    if(metisNumberSet_DecMax(pitEntry->ingressIdSet)) {
        return true;
    }
    // otherwise dec counter for all ingressIDs
    metisNumberSet_DecAllCounts(pitEntry->ingressIdSet);
    return false;
}
//----------------SAID----------------------------------------------------
