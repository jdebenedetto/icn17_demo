...
//--------------------------------SAID-------------------------------------------------
/**
 * @function metisPitEntry_AddOrUpdateIngressId
 * @abstract Increment the PRcount of an ingress connection id to the list of reverse paths
 * @discussion
 *   A PitEntry has two NumberSets.  The first is the set of ingress ports, which
 *   make up the reverse path.  The second is the set of egress ports, which make up
 *   its forward path. For ANP interests, each ingress port has a counter associated.
 *
 *   This function tracks which reverse paths have sent us the interest.
 *
 * @param ingressId the reverse path
 * @return true if the IngressId already exists, false otherwise
 */
 bool metisPitEntry_AddOrUpdateIngressId(MetisPitEntry *pitEntry, unsigned ingressId);

 /**
 * @function metisPitEntry_AfterSatisfy
 * @abstract Decrement the PRcount of ingress connections id to the list of reverse paths.
 * @discussion
 *
 *
 * @param 
 * @return true if the PitEntry has to be deleted from the PIT, false otherwise
 */
 bool metisPitEntry_AfterSatisfy(MetisPitEntry *pitEntry);
 //--------------------------------SAID-------------------------------------------------
