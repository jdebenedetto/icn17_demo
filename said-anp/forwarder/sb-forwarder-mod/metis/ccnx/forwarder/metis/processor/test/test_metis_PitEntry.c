...
LONGBOW_TEST_FIXTURE(Global)
{
    ...
    LONGBOW_RUN_TEST_CASE(Global, metisPitEntry_AddOrUpdateIngressId);
...
LONGBOW_TEST_CASE(Global, metisPitEntry_AddIngressId)
{
    ...
    MetisMessage *interest = metisMessage_CreateFromArray(metisTestDataV1_Interest_ANP, sizeof(metisTestDataV1_Interest_ANP), 1, 2, logger);
    metisLogger_Release(&logger);
...
LONGBOW_TEST_CASE(Global, metisPitEntry_AddOrUpdateIngressId)
{
    PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    parcLogReporter_Release(&reporter);
    ...
    //MetisPitEntry_Create also add IngressId (=1)
    MetisPitEntry *entry = metisPitEntry_Create(metisMessage_Acquire(interest), 10000, 0);

    bool add_1 = metisPitEntry_AddOrUpdateIngressId(entry, 1);
    bool add_10 = metisPitEntry_AddOrUpdateIngressId(entry, 10);
    bool add_11 = metisPitEntry_AddOrUpdateIngressId(entry, 11);

    bool updateMax2 = metisNumberSet_UpdateMax(entry->ingressIdSet, 2);
    bool updateMax3 = metisNumberSet_UpdateMax(entry->ingressIdSet, 3);

    metisPitEntry_Release(&entry);
    metisMessage_Release(&interest);


    assertTrue(add_1, "add_1 must be true");
    assertFalse(add_10, "add_1 must be true");
    assertFalse(add_11, "add_1 must be true");

    assertFalse(updateMax2, "Max is 2, don't need to update");
    assertTrue(updateMax3, "Max is 2, need to update to 3");
