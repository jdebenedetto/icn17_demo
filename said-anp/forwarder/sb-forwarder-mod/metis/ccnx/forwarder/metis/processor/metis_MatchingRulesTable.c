...
struct metis_matching_rules_table {
    ...
    PARCHashCodeTable *tableByNameANP;//SAID extra table
...
metisMatchingRulesTable_Create(PARCHashCodeTable_Destroyer dataDestroyer)
{
    ...
    table->tableByNameANP = parcHashCodeTable_Create_Size(metisHashTableFunction_MessageNameANPEquals,
                                                          metisHashTableFunction_MessageNameANPHashCode,
                                                          NULL,
                                                          dataDestroyer,
                                                          initialSize);
...
metisMatchingRulesTable_Destroy(MetisMatchingRulesTable **tablePtr)
{
    ...
    parcHashCodeTable_Destroy(&table->tableByNameANP); //SAID
...
metisMatchingRulesTable_GetUnion(const MetisMatchingRulesTable *table, const MetisMessage *message)
{
    ...
    // PARCArrayList *list = parcArrayList_Create_Capacity(NULL, NULL, 4); //SAID
    ...
    // //-----------------------------SAID--------------------------------------------
    // void *dataByNameANP = parcHashCodeTable_Get(table->tableByNameANP, message);
    // if (dataByNameANP) {
    //     parcArrayList_Add(list, dataByNameANP);
    // }
    // //-----------------------------SAID--------------------------------------------
...
//-----------------------------SAID--------------------------------------------
PARCArrayList *
metisMatchingRulesTable_GetANP(const MetisMatchingRulesTable *table, const MetisMessage *message)
{ 
    PARCArrayList *list = parcArrayList_Create_Capacity(NULL, NULL, 1);
...
metisMatchingRulesTable_RemoveFromBest(MetisMatchingRulesTable *rulesTable, const MetisMessage *message)
{
    ...
    PARCHashCodeTable *hashTable = metisMatchingRulesTable_GetTableForMessage(rulesTable, message);
    // printf("metisMatchingRulesTable_GetTableForMessage\n"); //DEBUG    
    parcHashCodeTable_Del(hashTable, message);
    // printf("parcHashCodeTable_Del\n"); //DEBUG
...
metisMatchingRulesTable_RemoveFromAll(MetisMatchingRulesTable *rulesTable, const MetisMessage *message)
{
    ...
    parcHashCodeTable_Del(rulesTable->tableByNameANP, message); //SAID
}
...
metisMatchingRulesTable_AddToAllTables(MetisMatchingRulesTable *rulesTable, MetisMessage *key, void *data)
{
    ...
    parcHashCodeTable_Add(rulesTable->tableByNameANP, key, data);
...
metisMatchingRulesTable_GetTableForMessage(const MetisMatchingRulesTable *pit, const MetisMessage *interestMessage)
{
    ...
    //--------------SAID----------------------------------
    if(metisMessage_GetFlags(interestMessage) == 1) {
        table = pit->tableByNameANP;
        // printf("tableByNameANP\n"); //DEBUG
    }
    //--------------SAID----------------------------------
    else if (metisMessage_HasContentObjectHash(interestMessage)) {
        // printf("tableByNameAndObjectHash\n"); //DEBUG
        table = pit->tableByNameAndObjectHash;
    } else if (metisMessage_HasKeyId(interestMessage)) {
        table = pit->tableByNameAndKeyId;
        // printf("tableByNameAndKeyId\n"); //DEBUG
    } else {
