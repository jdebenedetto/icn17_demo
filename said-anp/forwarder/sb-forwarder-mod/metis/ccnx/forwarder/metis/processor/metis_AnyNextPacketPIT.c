...
struct metis_anp_pit;
typedef struct metis_anp_pit MetisAnyNextPacketPIT;
...
static void _metisPIT_StoreInTable(MetisAnyNextPacketPIT *pit, MetisMessage *interestMessage);
...
static MetisTicks
_metisPIT_CalculateLifetime(MetisAnyNextPacketPIT *pit, MetisMessage *interestMessage)
{
...
static void
_metisPIT_StoreInTable(MetisAnyNextPacketPIT *pit, MetisMessage *interestMessage)
{
...
static void
_metisPIT_ExtendLifetime(MetisAnyNextPacketPIT *pit, MetisPitEntry *pitEntry, MetisMessage *interestMessage)
{
    MetisTicks expiryTime = _metisPIT_CalculateLifetime(pit, interestMessage);
...
static void
_metisAnyNextPacketPIT_Destroy(MetisPIT **pitPtr)
{
    ...
    MetisAnyNextPacketPIT *pit = metisPIT_Closure(*pitPtr);
...
_metisAnyNextPacketPIT_ReceiveInterest(MetisPIT *generic, MetisMessage *interestMessage)
{
    assertNotNull(generic, "Parameter pit must be non-null");
    assertNotNull(interestMessage, "Parameter interestMessage must be non-null");
    ...
    if (pitEntry) {
        ...
        // if (true) {//hack
            ...
            //-----------------------SAID--------------------------------------------------------------------
            // if it is an ANP Interest, we want to check&update in one operation to be more efficient
            uint8_t flags = metisMessage_GetFlags(interestMessage);
            // printf("flags = %d\n", flags); //DEBUG
            if (flags == 1) { //ANP
                //The function returns true if the interest need to be forwarded, otherwise false
                if(metisPitEntry_AddOrUpdateIngressId(pitEntry, metisMessage_GetIngressConnectionId(interestMessage))) {
                    if (metisLogger_IsLoggable(pit->logger, MetisLoggerFacility_Processor, PARCLogLevel_Debug)) {
                        metisLogger_Log(pit->logger, MetisLoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                                        "ANP Interest %p existing entry (expiry %" PRIu64 ") and reverse path, forwarding",
                                        (void *) interestMessage,
                                        metisPitEntry_GetExpiryTime(pitEntry));
                    }
                ...
                if (metisLogger_IsLoggable(pit->logger, MetisLoggerFacility_Processor, PARCLogLevel_Debug)) {
                    metisLogger_Log(pit->logger, MetisLoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                                    "ANP Interest %p existing entry (expiry %" PRIu64 ") and reverse path, aggregating",
                                    (void *) interestMessage,
                                    metisPitEntry_GetExpiryTime(pitEntry));
                }
...
_metisAnyNextPacketPIT_SatisfyInterest(MetisPIT *generic, const MetisMessage *objectMessage)
{
    assertNotNull(generic, "Parameter pit must be non-null");
    ...
    //---------------------SAID---------------------------------------------------------
    if(parcArrayList_Size(list) == 0) {
        parcArrayList_Destroy(&list);
        list = metisMatchingRulesTable_GetANP(pit->table, objectMessage);
    }
    //---------------------SAID---------------------------------------------------------
    for (size_t i = 0; i < parcArrayList_Size(list); i++) {
        ...
        //---------------------SAID--------------------------------------
        uint8_t flags = metisMessage_GetFlags(key);
        if (flags == 1) { //ANP
            if(metisPitEntry_AfterSatisfy(pitEntry)) {
...
static void
_metisAnyNextPacketPIT_RemoveInterest(MetisPIT *generic, const MetisMessage *interestMessage)
{
    assertNotNull(generic, "Parameter pit must be non-null");
    assertNotNull(interestMessage, "Parameter interestMessage must be non-null");
...
_metisAnyNextPacketPIT_GetPitEntry(const MetisPIT *generic, const MetisMessage *interestMessage)
{
    assertNotNull(generic, "Parameter pit must be non-null");
    assertNotNull(interestMessage, "Parameter interestMessage must be non-null");
...
MetisPIT *
metisAnyNextPacketPIT_Create(MetisForwarder *metis)
{
    assertNotNull(metis, "Parameter must be non-null");

    size_t allocation = sizeof(MetisPIT) + sizeof(MetisAnyNextPacketPIT);
    ...
    generic->getPitEntry = _metisAnyNextPacketPIT_GetPitEntry;
    generic->receiveInterest = _metisAnyNextPacketPIT_ReceiveInterest;
    generic->release = _metisAnyNextPacketPIT_Destroy;
    generic->removeInterest = _metisAnyNextPacketPIT_RemoveInterest;
    generic->satisfyInterest = _metisAnyNextPacketPIT_SatisfyInterest;
