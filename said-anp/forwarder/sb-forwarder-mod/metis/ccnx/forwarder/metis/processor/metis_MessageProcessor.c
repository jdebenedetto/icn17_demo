...
#include <ccnx/forwarder/metis/processor/metis_AnyNextPacketPIT.h>
...
typedef struct metis_processor_stats {
    ...
    uint32_t countANPInterestsReceived; //SAID
...
metisMessageProcessor_Create(MetisForwarder *metis)
{
    ...
    processor->pit = metisAnyNextPacketPIT_Create(metis);//SAID
...
metisMessageProcessor_ReceiveInterest(MetisMessageProcessor *processor, MetisMessage *interestMessage)
{
    ...
    //----------------SAID-----------------------------
    bool anp = false;
    if (metisMessage_HasFlags(interestMessage)) {
        uint8_t flags = metisMessage_GetFlags(interestMessage);
        // printf("Flag = %d\t", flags);//DEBUG
        if (flags == 1) {//ANP flag value is 1
            processor->stats.countANPInterestsReceived++;
            anp = true;
        }
    }
    //---------------SAID------------------------------
    ...
    if (!anp && _satisfyFromContentStore(processor, interestMessage)) { //SAID
        // done
...
metisMessageProcessor_ReceiveContentObject(MetisMessageProcessor *processor, MetisMessage *message)
{
    ...
    // printf("IngressSetUnion Length: %d\n", metisNumberSet_Length(ingressSetUnion)); //DEBUG
...
metisMessageProcessor_ForwardToNexthops(MetisMessageProcessor *processor, MetisMessage *message, const MetisNumberSet *nexthops)
{
    ...
    // printf("Forwarded Copies: %d\n", forwardedCopies); //DEBUG
