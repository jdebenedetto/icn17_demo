...
LONGBOW_TEST_FIXTURE(Global)
{
    ...
    LONGBOW_RUN_TEST_CASE(Global, metisMessage_ANPName); //SAID
    ...
    LONGBOW_RUN_TEST_CASE(Global, metisMessage_HasFlags_True);
    LONGBOW_RUN_TEST_CASE(Global, metisMessage_HasFlags_False);
    LONGBOW_RUN_TEST_CASE(Global, metisMessage_GetFlags);
    LONGBOW_RUN_TEST_CASE(Global, metisMessage_SetFlags);
...
//--------------------SAID----------------------------------------------------
LONGBOW_TEST_CASE(Global, metisMessage_ANPName)
{
    PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    parcLogReporter_Release(&reporter);
    ...
    MetisMessage *message1 = metisMessage_CreateFromArray(metisTestDataV1_Interest_AllFields, sizeof(metisTestDataV1_Interest_AllFields), 1, 2, logger);
    MetisMessage *message2 = metisMessage_CreateFromArray(metisTestDataV1_Interest_AllFields, sizeof(metisTestDataV1_Interest_AllFields), 1, 2, logger);
    metisLogger_Release(&logger);
    MetisMessage_SetName(message1, ccnxName1);
    MetisMessage_SetName(message2, ccnxName2);

    MetisTlvName *name1 = metisMessage_GetName(message1);
    MetisTlvName *name2 = metisMessage_GetName(message2);
    MetisTlvName* anpName1 = metisMessage_GetANPName(message1);
    MetisTlvName* anpName2 = metisMessage_GetANPName(message2);

    assertFalse(metisTlvName_Equals(name1, name2), "Name did not equal expected");
    assertTrue(metisTlvName_Equals(anpName1, anpName2), "ANPName did not equal expected");
    
    metisTlvName_Release(&anpName1);
    metisTlvName_Release(&anpName2);
    
    metisMessage_Release(&message1);
...
//----------------------------------SAID----------------------------------------------------------------
LONGBOW_TEST_CASE(Global, metisMessage_HasFlags_True)
{
    PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    parcLogReporter_Release(&reporter);
    MetisMessage *message = metisMessage_CreateFromArray(metisTestDataV1_Interest_AllFields, sizeof(metisTestDataV1_Interest_AllFields), 1, 2, logger);
    metisLogger_Release(&logger);

    bool hasFlags = metisMessage_HasFlags(message);
    assertTrue(hasFlags, "Message with a flags value says it does not.");

    metisMessage_Release(&message);
}

LONGBOW_TEST_CASE(Global, metisMessage_HasFlags_False)
{
    PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    parcLogReporter_Release(&reporter);
    MetisMessage *message = metisMessage_CreateFromArray(metisTestDataV1_Interest_NoFlags, sizeof(metisTestDataV1_Interest_NoFlags), 1, 2, logger);
    metisLogger_Release(&logger);

    bool hasFlags = metisMessage_HasFlags(message);
    assertTrue(hasFlags, "Message without a flags value says it does.");

    uint8_t zeroFlag = metisMessage_GetFlags(message);
    assertTrue(zeroFlag == 0, "Wrong flags value, got %u expected %u.", zeroFlag, 0);

    metisMessage_Release(&message);
}

LONGBOW_TEST_CASE(Global, metisMessage_GetFlags)
{
    PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    parcLogReporter_Release(&reporter);
    ...
    uint8_t flags = metisMessage_GetFlags(message);
    assertTrue(flags == 17, "Wrong flags value, got %u expected %u.", flags, 17);

    metisMessage_Release(&message);
}

LONGBOW_TEST_CASE(Global, metisMessage_SetFlags)
{
    // PARCLogReporter *reporter = parcLogReporterTextStdout_Create();
    // MetisLogger *logger = metisLogger_Create(reporter, parcClock_Wallclock());
    // parcLogReporter_Release(&reporter);
