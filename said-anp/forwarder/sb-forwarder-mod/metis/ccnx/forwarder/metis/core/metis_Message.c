...
//-----------------------SAID-mod-----------------------------------------------
bool
metisMessage_HasFlags(const MetisMessage *message)
{
    assertNotNull(message, "Parameter must be non-null");
    MetisTlvExtent extent = metisTlvSkeleton_GetFlags(&message->skeleton);
...
uint8_t
metisMessage_GetFlags(const MetisMessage *message)
{
    // bool hasFlags = metisMessage_HasFlags(message);
    // assertTrue(hasFlags, "Message does not have a Flags field");

    MetisTlvExtent extent = metisTlvSkeleton_GetFlags(&message->skeleton);
    uint8_t flags = message->messageHead[extent.offset];
    return flags;
}

//need it? forwarder has to be able to set a Flag?
void
metisMessage_SetFlags(MetisMessage *message, uint8_t flags)
{
    // assertNotNull(message, "Parameter must be non-null");
    // metisTlvSkeleton_UpdateFlags(&message->skeleton, flags);
}
//-----------------------SAID-mod-----------------------------------------------
...
//-----------------------SAID--------------------------------------------------
MetisTlvName *
metisMessage_GetANPName(const MetisMessage *message)
{
    assertNotNull(message, "Parameter message must be non-null");
    size_t seg_count = metisTlvName_SegmentCount(message->name);
    if(seg_count > 1) {
        return metisTlvName_Slice(message->name, seg_count - 1);
        // return metisTlvName_Slice(message->name, seg_count); //to be used until PRcount is not implemented
    }
    return message->name;
}
//-----------------------SAID--------------------------------------------------
...
//----------------------TESTING----------------------------------------------------
void
MetisMessage_SetName(MetisMessage *message, CCNxName* name)
{
    message->hasName = true;
    metisTlvName_Release(&message->name);
    message->name = metisTlvName_CreateFromCCNxName(name);
}
//----------------------TESTING----------------------------------------------------
