...
//-----------------------SAID-----------------------------------------
/**
 * Determines if the message has a flag
 *
 * <#Paragraphs Of Explanation#>
 *
 * @param [<#in out in,out#>] <#name#> <#description#>
 *
 * @retval true A flag exists
 * @retval false A flag does not exist
 *
 * Example:
 * @code
 * <#example#>
 * @endcode
 */
bool metisMessage_HasFlags(const MetisMessage *message);
 ...
 *
 * Will assert if the message does not have a flag.  Use metisMessage_HasFlags() first
 * to determine if there is a flag.
 *
 * @param [in] message An allocated MetisMessage
 *
 * @retval number The flags value
 *
 * Example:
 * @code
 * {
 *    if (metisMessage_HasFlags(message)) {
 ...
 * @endcode
 */
uint8_t metisMessage_GetFlags(const MetisMessage *message);

/**
 * Sets the message flags to the specified value
 *
 * Will assert if the message does not already have a flag. Use metisMessage_HasFlags() first
 * to determine if there is a flag.
 *
 * @param [in] message An allocated MetisMessage
 * @param [in] flags The value to set in the packet
 *
 * Example:
 * @code
 * {
 ...
 *          // do something
 *       } else {
 *          metisMessage_SetFlags(message, 0);
 *       }
 *    }
 * }
 * @endcode
 */
void metisMessage_SetFlags(MetisMessage *message, uint8_t flags);
...
/**
 * @function metisMessage_GetANPName
 * @abstract The AnyNextacket name in the CCNx message
 * @discussion
 *   The name of the Interest or Content Object.  If the caller will store the
 *   name, he should make a reference counted copy.
 *
 ...
 * @return The name as stored in the message object.
 */
 MetisTlvName *metisMessage_GetANPName(const MetisMessage *message);
...
void MetisMessage_SetName(MetisMessage *message, CCNxName* name); //TESTING
