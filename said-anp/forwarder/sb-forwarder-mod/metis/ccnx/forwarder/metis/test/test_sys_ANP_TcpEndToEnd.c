...
setupTcp(void)
{
    ...
    setupInetListener(globalState.metis, 50001);
    setupInetListener(globalState.metis, 50002);

    // create two test connections and learn their connection IDs via
    // the messenger callback

    globalState.fd_sender = setupInetClient(globalState.metis, 50001);
    metisDispatcher_RunCount(metisForwarder_GetDispatcher(globalState.metis), 1);
    printf("DEBUG sender   connection id = %u\n", testNotifierData.connectionid);

    globalState.fd_receiver = setupInetClient(globalState.metis, 50002);
    metisDispatcher_RunCount(metisForwarder_GetDispatcher(globalState.metis), 1);
    unsigned receiverConnectionId = testNotifierData.connectionid;
    printf("DEBUG receiver connection id = %u\n", testNotifierData.connectionid);
...
LONGBOW_TEST_FIXTURE(Global)
{
    ...
    // LONGBOW_RUN_TEST_CASE(Global, twoInterests);
    // LONGBOW_RUN_TEST_CASE(Global, twoANPInterests);
}
...
LONGBOW_TEST_CASE(Global, passInterest)
{
    ...
    ssize_t write_length = write(globalState.fd_sender, metisTestDataV1_Interest_NameA_Crc32c, sizeof(metisTestDataV1_Interest_NameA_Crc32c));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c),
               "Partial write, expected %zd got %zd: (%d) %s",
               sizeof(metisTestDataV1_Interest_NameA_Crc32c), write_length, errno, strerror(errno));
    ...
    assertTrue(read_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c), "Incorrect read, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), read_length, errno, strerror(errno));

    // We decrement the hoplimit, so need a new truth value
    uint8_t truth[1024];
    memcpy(truth, metisTestDataV1_Interest_NameA_Crc32c, sizeof(metisTestDataV1_Interest_NameA_Crc32c));
    // truth[12] = 31;
...
LONGBOW_TEST_CASE(Global, returnContentObject)
{
    ...
    ssize_t write_length = write(globalState.fd_sender, metisTestDataV1_Interest_NameA_Crc32c, sizeof(metisTestDataV1_Interest_NameA_Crc32c));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c), "Partial write of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), write_length, errno, strerror(errno));

    // run for a duration so there is time to read the message, pass it off to the handler, then send the message out
    metisDispatcher_RunDuration(metisForwarder_GetDispatcher(globalState.metis), &((struct timeval) { 0, 1000 }));

    ssize_t read_length = read(globalState.fd_receiver, receiveBuffer, 1024);
    assertTrue(read_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c), "Incorrect read of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), read_length, errno, strerror(errno));

    // send content object back
    write_length = write(globalState.fd_receiver, metisTestDataV1_ContentObject_NameA_Crc32c, sizeof(metisTestDataV1_ContentObject_NameA_Crc32c));
    assertTrue(write_length == sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), "Partial write of object, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), write_length, errno, strerror(errno));

    metisDispatcher_RunDuration(metisForwarder_GetDispatcher(globalState.metis), &((struct timeval) { 0, 1000 }));

    // now check that we got the object
    read_length = read(globalState.fd_sender, receiveBuffer, 1024);
    assertTrue(read_length == sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), "Incorrect read of object, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), read_length, errno, strerror(errno));

    assertTrue(memcmp(receiveBuffer, metisTestDataV1_ContentObject_NameA_Crc32c, read_length) == 0, "Objects do not match");
}

LONGBOW_TEST_CASE(Global, twoInterests)
{
    uint8_t receiveBuffer[1024];

    // send two ANP interests on the sender and see if we get two contentObject back
    // first interest
    ssize_t write_length = write(globalState.fd_sender, metisTestDataV1_Interest_NameA_Crc32c, sizeof(metisTestDataV1_Interest_NameA_Crc32c));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c), "Partial write of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), write_length, errno, strerror(errno));
    ...
    // second interest
    write_length = write(globalState.fd_sender, metisTestDataV1_Interest_NameA_Crc32c, sizeof(metisTestDataV1_Interest_NameA_Crc32c));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c), "Partial write of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), write_length, errno, strerror(errno));
    ...
    ssize_t read_length = read(globalState.fd_receiver, receiveBuffer, 1024);
    assertTrue(read_length == sizeof(metisTestDataV1_Interest_NameA_Crc32c)*2, "Incorrect read of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_NameA_Crc32c), read_length, errno, strerror(errno));
    ...
    // TO-DO: TO END THIS TEST A TIMEOUT MUST BE IMPLEMENT BEFORE THE READ() OPERATION
    // // send second content object back
    // write_length = write(globalState.fd_receiver, metisTestDataV1_ContentObject_NameA_Crc32c, sizeof(metisTestDataV1_ContentObject_NameA_Crc32c));
    // assertTrue(write_length == sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), "Partial write of object, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_ContentObject_NameA_Crc32c), write_length, errno, strerror(errno));
...
LONGBOW_TEST_CASE(Global, twoANPInterests)
{
    uint8_t receiveBuffer[1024];

    // send two ANP interests on the sender and see if we get two contentObject back
    // first interest
    ssize_t write_length = write(globalState.fd_sender, metisTestDataV1_Interest_ANP, sizeof(metisTestDataV1_Interest_ANP));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_ANP), "Partial write of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_ANP), write_length, errno, strerror(errno));
    
    // // run for a duration so there is time to read the message, pass it off to the handler, then send the message out
    // metisDispatcher_RunDuration(metisForwarder_GetDispatcher(globalState.metis), &((struct timeval) { 0, 1000 }));
    
    // second interest
    write_length = write(globalState.fd_sender, metisTestDataV1_Interest_ANP, sizeof(metisTestDataV1_Interest_ANP));
    assertTrue(write_length == sizeof(metisTestDataV1_Interest_ANP), "Partial write of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_ANP), write_length, errno, strerror(errno));
    ...
    ssize_t read_length = read(globalState.fd_receiver, receiveBuffer, 1024);
    assertTrue(read_length == sizeof(metisTestDataV1_Interest_ANP)*2, "Incorrect read of interest, expected %zd got %zd: (%d) %s", sizeof(metisTestDataV1_Interest_ANP), read_length, errno, strerror(errno));
