...
#define MetisTlvSkeleton_ArrayLength 13
...
//-----------------------------SAID--------------------------------------------------------------
/**
 * Sets the Flags extent
 *
 * For an Interest, the flags is found in the fixed header (v1)
 *
 * @param [in] skeleton A MetisTlvSkeleton structure
 * @param [in] offset The byte offset of the beginning of the 'value'
 * @param [in] length The byte length of the 'value'
 ...
 * {
 *    // For a version 1 packet, point it to the field in the fixed header
 *    metisTlvSkeleton_SetFlags(skeleton, 6, 1);
 * }
 * @endcode
 */
void metisTlvSkeleton_SetFlags(MetisTlvSkeleton *skeleton, size_t offset, size_t length);
//-----------------------------SAID--------------------------------------------------------------
...
//----------------------------------SAID------------------------------------------------
/**
 * Returns the extent of the Flags
 *
 * Returns the previously set Flags extent or metisTlvExtent_NotFound if not set.
 *
 * @param [in] skeleton An initialized and parsed skeleton
 *
 * @retval {0,0} Not Present
 ...
 *    MetisTlvSkeleton skeleton;
 *    if (metisTlvSkeleton_Parse(&skeleton, packet)) {
 *       MetisTlvExtent extent = metisTlvSkeleton_GetFlags(&skeleton);
 *    }
 * }
 * @endcode
 */
MetisTlvExtent metisTlvSkeleton_GetFlags(const MetisTlvSkeleton *skeleton);
//----------------------------------SAID-----------------------------------------------
