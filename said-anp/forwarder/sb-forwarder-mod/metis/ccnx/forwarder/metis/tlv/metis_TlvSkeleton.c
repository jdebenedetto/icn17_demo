...
//------------------------------SAID--------------------------------------------------------------------
void
metisTlvSkeleton_SetFlags(MetisTlvSkeleton *opaque, size_t offset, size_t length)
{
    _InternalSkeleton *skeleton = (_InternalSkeleton *) opaque;
    skeleton->array[INDEX_FLAGS].offset = offset;
    skeleton->array[INDEX_FLAGS].length = length;

    if (metisLogger_IsLoggable(skeleton->logger, MetisLoggerFacility_Message, PARCLogLevel_Debug)) {
        metisLogger_Log(skeleton->logger, MetisLoggerFacility_Message, PARCLogLevel_Debug, __func__,
                        "Set flags extent {%u, %u}", offset, length);
    }
}
//------------------------------SAID--------------------------------------------------------------------
...
//---------------------SAID-----------------------------------------
MetisTlvExtent
metisTlvSkeleton_GetFlags(const MetisTlvSkeleton *skeleton)
{
    return skeleton->array[INDEX_FLAGS];
}
//---------------------SAID-----------------------------------------
